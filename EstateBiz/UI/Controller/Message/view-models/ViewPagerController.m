...
#define kDefaultTabOffset 100 // Offset of the second and further tabs' from left
#define kDefaultTabWidth 60
#define kDefaultSpace 90//默认间距20
...
#define kDefaultIndicatorColor UIColorFromRGB(0x27A2F0)
#define kDefaultTabsViewBackgroundColor [UIColor whiteColor] //标签栏背景颜色
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    UILabel *labels = (UILabel *)[[activeTabView subviews] firstObject];
    [labels setTextColor:UIColorFromRGB(0x333B46)];

    // Set to-be-active tab selected
    activeTabView = [self tabViewAtIndex:activeTabIndex];
    activeTabView.selected = YES;
    UILabel *label = (UILabel *)[[activeTabView subviews] firstObject];
    [label setTextColor:UIColorFromRGB(0x27A2F0)];
...
- (void)reloadData {
    ...
    for (int i = 0; i < _tabCount; i++) {
        ...
        if (_tabCount == 2) {
            ...
            NSLog(@"tabWidth %f",frame.size.width);
            
            tabView.frame = frame;
            
            [_tabsView addSubview:tabView];
            
            contentSizeWidth += SCREENWIDTH/2; // 间距0

        } else {
            frame.size.width = _tabWidth;
            NSLog(@"tabWidth %f",frame.size.width);
            ...
            [_tabsView addSubview:tabView];
            if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
                contentSizeWidth += [self.delegate viewPager:self valueForOption:ViewPagerOptionSpace withDefault:kDefaultSpace]; // 间距
            } else {
                contentSizeWidth +=90;
            }
        }
